...
module.exports = {
    ...
    onNight: function(p) {
        ...
        var output = `It is now night ${p.game.day}! Use the ***${pre}${this.command}*** command to ${this.commandText} (ex: *${pre}${this.command} fool*). ***${pre}cancel*** to cancel.`;
        ...
        if (p.args[0] == this.command) {
            ...
            if (canDoActionResult === true) {
                ...
                if (target && target.alive) {
                    if (target.id != p.player.id || this.canSelfTarget || this.canSelfTarget == null) {
                        ...
                        p.mafiabot.reply(p.message, `**You are ${this.commandGerund} <@${target.id}> tonight!** Type ***${pre}cancel*** to cancel.`);
                    } else {
    ...
    onForceNightAction: function(p) {
        ...
        if (!(this.canSelfTarget || this.canSelfTarget == null)) {
            possibleTargets = _.filter(possibleTargets, player => player.id != p.player.id);
        }
        if (this.mustDoAction && canDoActionResult === true && possibleTargets.length > 0) {
            ...
            p.mafiabot.sendMessage(p.player.id, `**The night action time limit ran out, so you are randomly ${this.commandGerund} <@${target.id}> tonight!** Hurry up next time...`);
        } else {
